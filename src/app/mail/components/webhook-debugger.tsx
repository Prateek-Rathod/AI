...
const WebhookDebugger = () => {
  ...
  // Prevent query from firing if accountId is empty
  const enabled = !!accountId && accountId.length > 0;

  const { data, isLoading, error, refetch } =
    api.webhooks.getWebhooks.useQuery(
      { accountId },
      {
        enabled,
      }
  ...
  const handleCreateWebhook = async () => {
    if (!enabled) return toast.error("No account selected");
          ...
          <DialogDescription>
            {!enabled && (
              <div className="text-red-500 text-sm my-4">
                No account selected.  
                This tool needs a valid accountId.
              </div>
            )}
            ...
            <div className="mt-4">
              ...
              <Button
                ...
                disabled={!newWebhookUrl || !enabled}
              >
                Create Webhook
              </Button>
            </div>
